...
final class WebSocketConnection: XCTestCase
{
    ...
    func testUpdateFlow2() async throws
    {
        let app = try await Application.make(.testing)
        app.databases.use(.sqlite(.memory), as: .sqlite)
...
//        app.leaf.configuration = .init(rootDirectory: "Tests/Resources/Views")
...
//                        
//                        let m1 = DummyModel1(id: modelID, text: "Init")
//                        let m2 = DummyModel2(id: modelID, text2: "Init")
//                        
//                        try? await m1.save(on: config.app.db)
//                        try? await m2.save(on: config.app.db)
//                        
//                        m1.text = "Init edited"
//                        m2.text2 = "init edited"
//                        
//                        try? await m1.save(on: config.app.db)
//                        try? await m2.save(on: config.app.db)
//                        
//                        try? await config.app.asyncShutdown()
                    }
        ...
        { ws in
            Task
            {
                ...
                // send component subscription message to client -> server
                try await ws.send(subscriptionMessage)
            }
            ...
            { ws, text in
                ...
                switch message
                {
                    ...
                    {
                        XCTAssertEqual(component, "DumbComp4133", "Update message for wrong component.")
                        XCTAssertEqual(id, modelID, "Update message for wrong component.")
                    }
